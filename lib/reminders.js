...
async function processTaskReminders() {
  ...
  try {
    ...
    for (const task of tasks) {
      results.processed++;
      const deadlineStr = typeof task.deadline === "string" ? task.deadline.slice(0, 10) : new Date(task.deadline).toISOString().slice(0, 10);

      const isOverdue = deadlineStr < todayStr;
      const isDueToday = deadlineStr === todayStr;
      ...
      if (isDueToday) {
        ...
        if (!alreadySent) {
          ...
          await createNotification({
            ...
            message: `"${task.title}" in team ${task.team_name} is due today.`,
