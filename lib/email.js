...
function getTransporter() {
  ...
  if (smtpUser && smtpPass) {
    return nodemailer.createTransport({
      ...
      auth: {
        user: smtpUser.trim(),
        pass: smtpPass.replace(/\s+/g, ""), // clean spaces in app password
      },
    });
  }
  return null;
 ...
 * 1. Gmail SMTP (if SMTP_USER and SMTP_PASS set)
...
async function sendEmail({ to, subject, html, text }) {
  ...
  const smtpUser = process.env.SMTP_USER;
  const senderEmail = process.env.SENDER_EMAIL || (smtpUser ? `ProMa <${smtpUser}>` : "onboarding@resend.dev");

  // 1. Dispatch via Gmail SMTP
  if (transporter) {
    try {
      const fromAddress = senderEmail.includes("<") ? senderEmail : `ProMa <${senderEmail}>`;
      ...
      console.error("[Gmail SMTP Error] Failed to send email via Gmail:", smtpErr.message);
...
async function sendTeamInviteEmail({ to, teamName, invitedBy, inviteUrl }) {
  const title = `You've been invited to join ${teamName}`;
  const html = emailLayout({
    title,
    content: `
      <p style="font-size: 15px; color: #334155; line-height: 1.5;">
        <strong>${invitedBy}</strong> has invited you to join the team <strong>${teamName}</strong> on ProMa.
      </p>
  ...
  return sendEmail({
    to,
    subject: `${invitedBy} invited you to join ${teamName} on ProMa`,
    html,
    text: `${invitedBy} has invited you to join ${teamName} on ProMa. Open ProMa to join: ${inviteUrl || APP_URL}`,
  });
}

// 3. Task Assignment Email
async function sendTaskAssignmentEmail({ to, recipientName, taskTitle, taskDescription, teamName, assignedBy, dueDate, status, taskUrl }) {
  ...
  const html = emailLayout({
    ...
    content: `
      ...
      <p style="font-size: 15px; color: #334155; line-height: 1.5;">
        <strong>${assignedBy}</strong> has assigned an activity to you on ProMa.
      </p>
      ...
      <div style="background-color: #f8fafc; border-left: 4px solid #3454D1; border-radius: 4px; padding: 16px; margin: 20px 0;">
        ...
        <table role="presentation" width="100%" style="font-size: 13px; color: #64748b;">
          ...
          <tr>
            <td style="padding: 2px 0;"><strong>Assigned by:</strong> ${assignedBy}</td>
          </tr>
        </table>
      </div>
    `,
  ...
  return sendEmail({
    to,
    subject: `New activity assigned: ${taskTitle}`,
    html,
    text: `You have been assigned "${taskTitle}" in ${teamName} by ${assignedBy}. Due: ${dueDate || "N/A"}.`,
  });
}
