...
async function sendEmail({ to, subject, html, text }) {
  ...
  if (transporter && smtpUser) {
    try {
      ...
      console.log(`[Email Sent] Successfully delivered email to ${Array.isArray(to) ? to.join(", ") : to} via SMTP (ID: ${info.messageId})`);
      return { ok: true, id: info.messageId, provider: "smtp" };
    } catch (smtpErr) {
      console.warn(`[SMTP Warning] SMTP send failed (${smtpErr.message}). Attempting HTTPS API fallback...`);
      if (!apiKey) {
  ...
  // 2. Dispatch via Resend HTTP API (Port 443 — works seamlessly on cloud hosts like Render/Vercel)
  if (apiKey) {
    try {
      ...
      if (!res.ok) {
        const errMsg = data.message || data.name || `Resend API error (status ${res.status})`;
        console.error(`[Resend API Error] Failed to deliver email to ${to}: ${errMsg}`, JSON.stringify(data));
        return { ok: false, error: errMsg };
      }
      console.log(`[Email Sent] Successfully delivered email to ${Array.isArray(to) ? to.join(", ") : to} via Resend HTTPS API (ID: ${data.id})`);
  ...
  // 3. Fallback to Console Dev Mode
  console.log(`\n========================================`);
