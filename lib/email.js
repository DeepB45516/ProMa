...
async function sendEmail({ to, subject, html, text }) {
  ...
  const senderEmail = process.env.SENDER_EMAIL || smtpUser || "noreply@promaapp.com";
  const senderName = "ProMa";

  // 1. Try Gmail SMTP if configured (works on localhost; may timeout on Render)
  ...
  // 2. Brevo REST API (HTTPS port 443 — works on Render, no domain verification needed)
  if (brevoKey) {
    try {
      ...
      if (!res.ok) {
        const errMsg = data.message || `Brevo API error (status ${res.status})`;
  ...
  // 3. Resend fallback (requires verified domain/email in Resend dashboard)
  if (resendKey) {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        // Log the error but fall through to the dev-mode console fallback so
        // local development still works when Resend is in sandbox mode (which
        // only allows sending to the account owner's verified email address).
        // In production you must configure a verified sender domain in Resend.
        console.error(`[Resend API Error] ${data.message || res.status}`, JSON.stringify(data));
  ...
  // 4. Dev Fallback — no email provider configured (or all providers failed)
  console.log(
    `\n==== [Email Dev Fallback] ====\nTo: ${to}\nSubject: ${subject}\n${text ? `Body: ${text}` : ""}\n=============================\n`
  );
